...
class D3TableMetadata(Base):
    ...
    all_editions: Mapped[list['D3EditionMetadata']] = relationship(back_populates="table")
...
class PrimaryEditions:
    id: Mapped[int] = mapped_column(primary_key=True)
    time_frame: Mapped[TimeFrame] = mapped_column(Enum(TimeFrame))
    table_id: Mapped[int] = mapped_column(ForeignKey("d3_table_metadata.id"))
...
def get_table_metadata(db: Session, table_name: str) -> Optional[D3TableMetadata]:
    """
