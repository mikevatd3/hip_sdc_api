...
def query_all_levels(db_connect_string):
    ...
    q = "SELECT DISTINCT sumlevel FROM tiger2016.census_name_lookup order by sumlevel;"
    cur.execute(q)
...
def query_one_level(level,db_connect_string):
    ...
    q = "SELECT display_name, full_geoid from tiger2016.census_name_lookup where sumlevel = '%s'" % (level)
    cur.execute(q)
    results = cur.fetchall()
